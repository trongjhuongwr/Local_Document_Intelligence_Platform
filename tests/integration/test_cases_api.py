...
async def test_case_crud_batch_upload_and_cascade(client) -> None:
    created = await client.post("/cases", json={"name": "Acme invoice review"})
    ...
    deleted = await client.delete(f"/cases/{case_id}")
    assert deleted.status_code == 204
    assert (await client.get(f"/cases/{case_id}")).status_code == 404


async def test_demo_case_is_idempotent(client) -> None:
    first = await client.post("/demo/cases")
    second = await client.post("/demo/cases")
...
async def test_review_filters_return_total(client) -> None:
    ...
    response = await client.get(
        "/reviews", params={"case_id": case_id, "severity": "high", "limit": 1}
    )

...
async def test_upload_duplicate_get_chunks_delete(client):
    ...
    listed = await client.get("/api/documents", params={"case_id": case_id})
    ...
    assert (await client.get(f"/api/documents/{document_id}/chunks")).status_code == 404
    assert list(UPLOAD_DIR.glob(f"{document_id}.*")) == []
    assert (await client.delete(f"/api/documents/{other_document_id}")).status_code == 204
...
async def test_list_filters(client):
    ...
    by_type = await client.get(
        "/api/documents", params={"case_id": case_id, "document_type": "policy"}
    )
    assert [item["document_id"] for item in by_type.json()] == [document_id]

    wrong_type = await client.get(
        "/api/documents", params={"case_id": case_id, "document_type": "invoice"}
    )
    assert wrong_type.json() == []

    by_status = await client.get("/api/documents", params={"case_id": case_id, "status": "parsed"})
    assert len(by_status.json()) == 1

    offset_page = await client.get("/api/documents", params={"case_id": case_id, "offset": 1})

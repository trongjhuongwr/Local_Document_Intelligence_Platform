...
Future eval modules (retrieval, routing, generation, workflow) are picked up
automatically when they expose ``async def run_for_run_all(max_cases) -> dict``.
"""
...
from evals.extraction.run import run_extraction_eval
...
async def run_all(max_cases: int | None, skip_llm: bool) -> dict[str, Any]:
    ...
    if llm_ready:
        results["extraction"] = await run_extraction_eval(max_cases=max_cases)
        results["discrepancy_end_to_end"] = await run_discrepancy_eval(
            "end_to_end", max_cases=max_cases
        )
    else:
...
def main(argv: list[str] | None = None) -> int:
    ...
    outcome = asyncio.run(run_all(max_cases=args.cases, skip_llm=args.skip_llm))

...
<body>
  <div class="header">
    <div style="float: right; text-align: right;">
      <span class="badge badge-ready">AUDIT DOSSIER</span>
    ...
    <tbody>
      ${auditLogs.slice(0, 10).map((log: any) => `
        <tr>
          ...
          <td>${log.action}</td>
          <td class="mono" style="font-size: 10px;">${log.integrity_hash ? log.integrity_hash.slice(0, 16) + '...' : '—'}</td>
        </tr>
      `).join('')}
    </tbody>
  </table>
  ...
  <div style="margin-top: 36px; border-top: 1px solid #cbd5e1; padding-top: 16px; display: flex; justify-content: space-between; font-size: 11px; color: #64748b;">
    <div>Generated by: <strong>Local Document Intelligence Platform</strong></div>
    <div>Exported: <span class="mono">${new Date().toISOString()}</span></div>
  </div>
